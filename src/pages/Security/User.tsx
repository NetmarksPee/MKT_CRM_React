...
export default function User() {
  ...
  const callApi = async () => {
    const apiUrl = import.meta.env.VITE_CRM_API_URL + "users";
  ...
  const [rows2, setUser] = useState(us);
  ...
  async function handlerDelete() {
    const apiUrl = import.meta.env.VITE_CRM_API_URL + "users/delete";
  ...
  const columns: GridColDef<(typeof rows2)[number]>[] = [
    { field: "user_name", headerName: "ID", width: 80 },
    {
      field: "password",
      headerName: "Password",
      width: 0,
...
`;
  ...
  const handleAdd = () => {
    ...
    navigate("/UserDetail", { state: { parameterValue } });
  };
              ...
              <div>
                ...
                <TransitionGroup>
                  <DataGrid
                    ...
                    rows={rows2}
                    columns={columns}
                    getRowId={(row: any) => row.user_name}
                    initialState={{
                      pagination: {
                        paginationModel: {
                          pageSize: 8,
